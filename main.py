...
from src.processing.transform   import (transform_activities, transform_laps, transform_best_efforts)
from src.enrichment.weather     import enrich_with_weather, backfill_weather
from src.utils.storage          import (save_raw_data, save_processed_snapshot, save_processed_data, get_last_activity_timestamp, CONSOLIDATED_PATH, save_laps_data,save_laps_snapshot, save_best_efforts_data, save_best_efforts_snapshot)
...
def main():
    ...
    raw_activities, raw_laps, raw_best_efforts = get_all_enriched_activities(access_token, after=after, fetch_details=True, fetch_streams=False, fetch_laps=True)

    print(f"\nColetado: "f"{len(raw_activities)} atividades | "f"{len(raw_laps)} laps | " f"{len(raw_best_efforts)} best_efforts")
...
def run_backfill():
    ...
    raw_laps, raw_be = backfill_laps_and_best_efforts(access_token, activity_ids, fetch_laps=True, fetch_best_efforts=True)
